...
class MapReader2 : MonoBehaviour 
{

    public GameObject popupTextObject;

    double latitude = 51.085235;
    double longitude = 17.054275;
    ...
    IEnumerator DownloadAndLoadOSMData()
    {
        ...
        yield return StartCoroutine(DownloadOSMData());

        // Wczytaj dane z pamięci podręcznej
        string cachedData = LoadCachedOSMData();
    ...
    // Asynchroniczna metoda pobierająca dane OSM
    IEnumerator DownloadOSMData()
    {
        ...
        if (www.isNetworkError || www.isHttpError)
        {
            ...
            DisplayPopup("Błąd pobierania danych OSM: " + www.error);
            Debug.LogError("Błąd pobierania danych OSM: " + www.error);
        }
        else
        {
    ...
    // Metoda zapisująca dane OSM do pamięci podręcznej
    private void SaveOSMDataToCache(XmlDocument xmlDoc)
    {
        if (xmlDoc != null)
        {
            // Zapisz dane do pamięci podręcznej
            PlayerPrefs.SetString("OSMData", xmlDoc.OuterXml);
            PlayerPrefs.Save();

            Debug.Log("Dane OSM zostały zapisane w pamięci podręcznej.");

        }
        else
        {
            Debug.LogError("Błąd podczas parsowania danych OSM do XmlDocument.");
            DisplayPopup("Dane OSM zostały zapisane w pamięci podręcznej.");
        }
    }

    // Metoda wczytująca dane OSM z pamięci podręcznej
    string LoadCachedOSMData()
    {
        // Odczytaj dane z pamięci podręcznej
        if (PlayerPrefs.HasKey("OSMData"))
        {
            string osmXMLData = PlayerPrefs.GetString("OSMData");
            Debug.Log("Wczytane dane z pamięci podręcznej:\n" + osmXMLData);

            // Tutaj możesz przekazać osmXMLData do swojej klasy MapReader lub innego komponentu, który przetwarza dane OSM.
        ...
        else
        {
            ...
            if (popupTextObject != null)
            {
                ...
                if (popupText != null)
                {
                    popupText.text = "Brak wcześniej zapisanych danych w pamięci podręcznej.";
                }
            }
            Debug.Log("Brak wcześniej zapisanych danych w pamięci podręcznej.");
            return null; // Możesz zwrócić null lub inne oznaczenie braku danych
        }
    }
